  ...
  return (
    <div className="min-h-screen bg-roof-cream">
      ...
      <main>
        ...
        <section className="bg-roof-deep py-16 text-white"><div className="mx-auto grid max-w-7xl gap-8 px-5 sm:px-6 lg:grid-cols-2 lg:px-8"><div><p className="text-xs font-bold uppercase tracking-[0.24em] text-roof-sand">Our Service Area</p><h2 className="mt-3 text-3xl font-bold text-white">{`Local content for the wider ${business.areaServed.name}`}</h2></div><div className="space-y-3">{["Maitland and surrounding residential communities", "Cessnock and the vineyard districts", "Singleton, Muswellbrook and Scone", "Rural, residential and commercial properties"].map((item) => <div key={item} className="flex gap-3"><CheckCircle2 className="mt-0.5 h-5 w-5 text-roof-copper" /><p className="text-sm leading-7 text-white/70">{item}</p></div>)}</div></div></section>
      </main>
