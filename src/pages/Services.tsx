      ...
      <main>
        ...
        <section className="bg-roof-cream py-16 md:py-20">
          <div className="mx-auto grid max-w-7xl gap-10 px-5 sm:px-6 lg:grid-cols-2 lg:px-8">
            <div><p className="text-xs font-bold uppercase tracking-[0.24em] text-roof-copper">How to choose</p><h2 className="mt-3 text-3xl font-bold text-roof-deep">Which retaining wall type suits your site?</h2><p className="mt-4 text-base leading-8 text-roof-slate">The most suitable option depends on wall height, soil conditions, drainage requirements, load expectations and the desired appearance.</p></div>
            <div className="space-y-3">{["Block walls suit structural applications and can be rendered for a clean finish.", "Concrete sleepers offer durability and a modern look for gardens and driveways.", "Sandstone provides a natural aesthetic for established gardens and heritage sites.", "Timber can be cost-effective for lower walls in garden and landscape settings."].map((item) => <div key={item} className="flex gap-3 rounded-2xl bg-white p-4"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-roof-teal" /><p className="text-sm leading-7 text-roof-slate">{item}</p></div>)}</div>
          </div>
        </section>
      </main>
