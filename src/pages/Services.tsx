      ...
      <main>
        ...
        <section className="bg-brand-cream py-16 md:py-20">
          <div className="mx-auto grid max-w-7xl gap-10 px-5 sm:px-6 lg:grid-cols-2 lg:px-8">
            <div><p className="text-xs font-bold uppercase tracking-[0.24em] text-brand-gold">How to choose</p><h2 className="mt-3 text-3xl font-bold text-brand-deep">Repair, restore or replace?</h2><p className="mt-4 text-base leading-8 text-brand-slate">The most suitable option depends on how localised the damage is, the condition of surrounding materials, recurring failures, roof age and future plans for the property.</p></div>
            <div className="space-y-3">{["Targeted repairs can suit isolated defects on an otherwise serviceable roof.", "Restoration can suit certain roofs that need cleaning, repairs, ridge work and coating.", "Replacement may be more sensible when deterioration or corrosion is widespread.", "An inspection helps separate immediate work from longer-term improvements."].map((item) => <div key={item} className="flex gap-3 rounded-2xl bg-white p-4"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-brand-olive" /><p className="text-sm leading-7 text-brand-slate">{item}</p></div>)}</div>
          </div>
        </section>
      </main>
