    ...
    <section className="surface-grid bg-brand-cream py-16 md:py-24">
      <div className="mx-auto grid max-w-7xl items-center gap-10 px-5 sm:px-6 lg:grid-cols-2 lg:px-8">
        <div className="relative">
          ...
          <img src={homeImages.about} alt={`Retaining wall construction by ${business.name}`} className="relative aspect-[4/3] w-full object-cover shadow-[0_30px_80px_rgba(22,58,53,.16)]" />
          <div className="absolute bottom-5 left-5 right-5 flex items-start gap-3 bg-white/95 p-4 shadow-lg backdrop-blur"><MapPin className="mt-0.5 h-5 w-5 text-brand-gold" /><p className="text-sm leading-6 text-brand-slate">{`Serving ${business.areaServed.primaryLocationListText} and surrounding ${business.areaServed.name} communities.`}</p></div>
        </div>
        <div>
          ...
          <Link to="/about-us/" className="mt-7 inline-flex items-center gap-2 bg-brand-olive px-6 py-3.5 text-sm font-bold text-white transition hover:bg-brand-oliveDark">Learn about our approach <ArrowRight className="h-4 w-4" /></Link>
        </div>
      </div>
    </section>
  );
