...
export default function AboutUs() {
  ...
  return (
    <div className="min-h-screen bg-roof-cream">
      ...
      <Header />
      <main>
        <PageHero eyebrow="About Us" title="A practical approach to retaining wall construction" description={`${business.areaServed.name} properties range from suburban homes and sloped blocks to commercial developments and landscaping projects. We shape the retaining wall scope around the site, not a generic package.`} image="/images/services01.webp" />
        <section className="bg-white py-16 md:py-24">
          <div className="mx-auto grid max-w-7xl items-center gap-10 px-5 sm:px-6 lg:grid-cols-2 lg:px-8">
            <div><p className="text-xs font-bold uppercase tracking-[0.24em] text-roof-copper">Our Approach</p><h2 className="mt-3 text-3xl font-bold text-roof-deep sm:text-4xl">Clear information supports better construction decisions</h2><div className="mt-5 space-y-4 text-base leading-8 text-roof-slate"><p>Retaining wall projects can range from a straightforward garden bed wall to complex structural walls supporting buildings or managing significant level changes. We aim to make the next step easier to understand by assessing the site and explaining the scope in plain English.</p><p>Our service pages are structured to help homeowners, developers, landscapers and commercial clients find relevant information before making an enquiry.</p></div></div>
            <img src="/images/contact-workers.webp" alt={`${business.name} retaining wall team`} className="aspect-[4/3] w-full rounded-[32px] object-cover shadow-[0_28px_75px_rgba(22,58,53,.15)]" />
          </div>
        </section>
        ...
        <section className="bg-roof-deep py-16 text-white"><div className="mx-auto grid max-w-7xl gap-8 px-5 sm:px-6 lg:grid-cols-2 lg:px-8"><div><p className="text-xs font-bold uppercase tracking-[0.24em] text-roof-sand">Our Service Area</p><h2 className="mt-3 text-3xl font-bold text-white">{`Local coverage across ${business.areaServed.name}`}</h2></div><div className="space-y-3">{["Northern Beaches, Sutherland Shire and Eastern Suburbs", "Hills District, North Shore and Hornsby", "Parramatta, Blacktown and Western Sydney", "Campbelltown, Penrith and surrounding areas"].map((item) => <div key={item} className="flex gap-3"><CheckCircle2 className="mt-0.5 h-5 w-5 text-roof-copper" /><p className="text-sm leading-7 text-white/70">{item}</p></div>)}</div></div></section>
      </main>
