...
export default function FAQ() {
  ...
  return (
    <div className="min-h-screen bg-brand-cream">
      ...
      <Header />
      <main>
        <PageHero eyebrow="Frequently Asked Questions" title="Straight answers about retaining walls" description="Use these answers as a starting point, then contact us with the details of your site and project for advice relevant to your situation." image="/images/concretesleeper2.jpg" />
        <section className="bg-white py-16 md:py-24"><div className="mx-auto max-w-4xl px-5 sm:px-6 lg:px-8"><div className="space-y-4">{faqs.map((faq) => <details key={faq.question} className="group overflow-hidden border border-brand-mist bg-brand-cream/45"><summary className="flex cursor-pointer list-none items-center justify-between gap-5 px-5 py-5 font-bold text-brand-deep sm:px-6"><span>{faq.question}</span><span className="text-2xl font-light text-brand-gold transition group-open:rotate-45">+</span></summary><div className="px-5 pb-5 text-sm leading-7 text-brand-slate sm:px-6">{faq.answer}</div></details>)}</div></div></section>
      </main>
