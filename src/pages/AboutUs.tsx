...
export default function AboutUs() {
  ...
  return (
    <div className="min-h-screen bg-brand-cream">
      <SEO title={`About ${business.name} | Local Roofing Team`} description={`Learn about ${business.name}, our practical approach to roof repairs, maintenance, restoration and replacement, and the ${business.areaServed.name} areas we service.`} canonical={`${business.url}/about-us/`} />
      <Header />
      <main>
        <PageHero eyebrow="About Us" title="A practical roofing approach for a diverse region" description={`${business.areaServed.name} properties range from suburban homes and growing estates to vineyards, rural buildings, accommodation venues and commercial sites. We shape the roofing scope around the building, not a generic package.`} image="/images/standing-seam-metal-roof.webp" />
        <section className="bg-white py-16 md:py-24">
          <div className="mx-auto grid max-w-7xl items-center gap-10 px-5 sm:px-6 lg:grid-cols-2 lg:px-8">
            <div><p className="text-xs font-bold uppercase tracking-[0.24em] text-brand-gold">Our Approach</p><h2 className="mt-3 text-3xl font-bold text-brand-deep sm:text-4xl">Clear information supports better roofing decisions</h2><div className="mt-5 space-y-4 text-base leading-8 text-brand-slate"><p>Roofing work can range from a straightforward broken-tile repair to complex water entry, storm damage or a full replacement. We aim to make the next step easier to understand by identifying the likely issue and explaining the scope in plain English.</p><p>Our service pages and area pages are structured to help homeowners, landlords, property managers and business owners find relevant information before making an enquiry.</p></div></div>
        ...
        <section className="bg-brand-deep py-16 text-white"><div className="mx-auto grid max-w-7xl gap-8 px-5 sm:px-6 lg:grid-cols-2 lg:px-8"><div><p className="text-xs font-bold uppercase tracking-[0.24em] text-brand-sand">Our Service Area</p><h2 className="mt-3 text-3xl font-bold text-white">{`Local content for the wider ${business.areaServed.name}`}</h2></div><div className="space-y-3">{["Maitland and surrounding residential communities", "Cessnock and the vineyard districts", "Singleton, Muswellbrook and Scone", "Rural, residential and commercial properties"].map((item) => <div key={item} className="flex gap-3"><CheckCircle2 className="mt-0.5 h-5 w-5 text-brand-gold" /><p className="text-sm leading-7 text-white/70">{item}</p></div>)}</div></div></section>
      </main>
